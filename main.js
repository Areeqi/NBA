...
// الإصدار النهائي المتكامل – v9.1 (رفع إلى ImgBB مع ضغط وتقدم)
// ================================================================

import * as THREE from 'three';

// ================================================================
// إعدادات Firebase السحابية (Realtime Database فقط)
// ================================================================
...
// ================================================================
// 10. IMAGE UPLOADER – رفع إلى ImgBB (مجاني، مع ضغط وتقدم)
// ================================================================
...
export class ImageUploader {
  ...
  static compressImage(file, maxWidth = 1200, maxHeight = 1200, quality = 0.7) {
    return new Promise((resolve, reject) => {
      // إذا كان الملف صغيراً جداً (أقل من 200 كيلوبايت)، نرفعه دون ضغط
        ...
        img.onload = () => {
          try {
            ...
            canvas.toBlob((blob) => {
              ...
              console.log(`✅ تم ضغط الصورة: ${(file.size/1024).toFixed(1)} KB → ${(blob.size/1024).toFixed(1)} KB`);
              resolve(compressedFile);
            }, mimeType, quality);
          } catch (err) {
            reject(new Error(`خطأ في الضغط: ${err.message}`));
          }
        };
  ...
  // رفع ملف إلى ImgBB مع تتبع التقدم
  static async uploadFile(file, onProgress = null) {
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        xhr.onerror = () => reject(new Error('فشل الاتصال بخادم ImgBB'));
        xhr.send(formData);
      });
...
console.log('⚡ النخبة – النواة الأسطورية v9.1 (رفع إلى ImgBB مع ضغط وتقدم) جاهزة');
...
console.log('🖼️ ImageUploader: رفع إلى ImgBB مع ضغط تلقائي وشريط تقدم');
